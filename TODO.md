...
## היום האחרון לפני המשתמשת הראשונה (2026-09-25)
...
והסריקה שאחריהם מצאה שהספירה הזאת שיקרה בשתיים. *מודול* אכן עבר כולו, אבל
**אני** — החצי השני של הסרגל — לא מודול, ולכן נשמט: כרטיסים לבנים על רקע,
סגול, ולוח חזון במסגרת עץ. חמור מזה, הוא לא נשא חומר משלו בכלל: נמדד
בדפדפן ש-showMain('me') משאיר על ה-body את המחלקה של האזור האחרון, אז אותו
מסך עצמו נצבע בזכוכית של תזונה אם הגעת משם ובלבן הרך אם עברת קודם בבית.
שניהם נסגרו ב-25 בספטמבר.

**ורגרסיה שהכנסתי בעצמי באותו תיקון, שנמצאה כי חיפשתי אותה.** השמירה
האוטומטית שמרה גם כשלא נכתב כלום, ורשומה ריקה אינה כלום: `sumRemCheck`
קוראת `loadSummary(t)` כ"היום כבר סוכם" ומשתיקה את עצמה. כלומר פתיחת
הרפלקציה בשלוש אחר הצהריים בלי לכתוב הייתה מבטלת את תזכורת הערב — התיקון
לאובדן פסקה לקח בשקט את הדבר היחיד שהדלת מבטיחה. השומר נמצא ב-`rfHold`
ולא ב-`rfCommitText`, כי "המשיכי" באמת צריכה לכתוב ערך ריק.

**ומה שזה מלמד על המבנה, ורשום כדי שלא יישכח:** "האם היום סוכם" נמדד
בקיום הרשומה, לא בקיום תשובה. זה עבד כל עוד רשומה נוצרה רק בלחיצה. כל
כתיבה עתידית שאינה יזומה תיתקל באותו קיר.

**ושלישי, במסלול שרץ כשהיא לא מסתכלת מסוג אחר.** הקלדה בשאלה של הרפלקציה
לא כתבה כלום — התשובה חייתה בתיבה עד "המשיכי". בין שאלות הכול נשמר, אבל
הפסקה שהיא באמצעה לא, ובטלפון החלון הזה אינו נדיר: המסך ננעל, הדפדפן
זורק את הלשונית, ומה שכתבה הולך איתה. `rfCommitText` כבר עשתה את הנכון —
היא מה ש"המשיכי" קורא לו — אז היא נקראת עכשיו גם ב-`pagehide` וב-
`visibilitychange`, משומרת לתצוגת השלב בלבד. נמדד: לפני ההסתרה האחסון
ריק, אחריה המשפט שם, וטעינה מחדש מחזירה את התיבה מלאה.

**ומיד אחריו השני, שהוא אותו באג בחוליה הבאה.** ההתראה שמגיעה כשהאפליקציה
**סגורה** נכתבת ב-`sw.js`, לא באפליקציה — דחיפת ווב כאן חסרת מטען בכוונה,
כדי שהשרת לא יחזיק מה שאדם כתב. שם ישבו שני קבועים באנגלית, תחת הערה
שאומרת שהם "מסונכרנים עם SUMREM_MSG", מה שחדל להיות נכון באותו בוקר.
כלומר תיקנתי את ההתראה שמופיעה כשהיא כבר מסתכלת, ופספסתי את זו שעל מסך
הנעילה. האפליקציה מניחה עכשיו את המשפט ב-Cache שה-worker קורא, מתורגם
ובלשון שנבחרה, והקבועים נשארו כגיבוי בלבד. ועוד פער באותה שרשרת: המשפט
מאוחסן, אז הוא נשמר מחדש בכל שינוי שפה או לשון.

**ומה שלא נמדד, ורשום כדי שלא ייקרא כאילו כן:** דחיפה אמיתית אי אפשר
לירות מקומית. מה שנבדק הוא שהשרת עונה `ok` ב-`/health`, שהקרון רץ כל
דקה עם חלון איחור של שעתיים, שהאפליקציה מניחה את המשפט הנכון, ושה-worker
קורא אותו ומעדיף אותו. מסירה מקצה לקצה תיבדק רק בערב הראשון.

**והבאג החמור ביותר של היום נמצא אחרי האימות הזה.** הדלת שואלת מתי להזכיר
לה לעצור, ומבטיחה מתחת לשאלה ש"זה הדבר היחיד שהאפליקציה תעשה מיוזמתה".
`doorRemind` כתב את השעה לאחסון והלך הלאה: לא ביקש הרשאת התראות ולא קרא
ל-`sumRemPushSync`. כלומר ב-21:00 לא היה מגיע אליה דבר אלא אם האפליקציה
במקרה פתוחה — ההבטחה היחידה שהדלת נותנת הייתה הדבר היחיד שלא קורה.
המסלול דרך ההגדרות היה תקין תמיד; זו הייתה הדלת לבדה, שהיא המסלול היחיד
שהיא אי פעם תעבור. ובאותה פונקציה גם חסר השומר על `last`, כך שמענה על
הדלת אחרי השעה שנבחרה היה יורה את התזכורת על המסך הראשון שלה. שניהם
נסגרו ושוחררו.

**ומנגנון הרענון העצמי נבדק ונמצא חמוש.** האפליקציה משווה ETag כל חמש דקות
ובכל חזרה ללשונית, ולא מרעננת בזמן שמשהו מוקלד. מדידה חוצת-מקורות החזירה
`etag: null` והייתה נראית כמו תקלה — היא ארטיפקט: ETag אינו כותרת מותרת
ב-CORS. מתוך האפליקציה עצמה, מאותו מקור, הוא שם, ו-`_verTag` החי כבר
מחזיק אותו. Pages מחזיר גם `last-modified`, שהקוד נופל אליו ממילא.

**אומת מקצה לקצה על הבילד המפורסם, 25 בספטמבר בבוקר.** ממאגר ריק, בעברית,
בלשון נקבה — כלומר בדיוק מה שהיא תפתח הערב:

- הדלת רצה חמישה מסכים ונחתה בסיום יום. נשמרו: שם, מגדר, אזורים ו-21:00.
- סיום יום: "עוד לא עצרת לסכם את היום" · "התחילי סיכום" ·
  "בואי ניקח רגע להסתכל על היום שלך" · "דלגי" · "המשיכי".
- מסך הבית: "בוקר נפלא, מיכל" עם האזורים שהיא בחרה בלבד.
- תשעת האזורים, הבית ו"אני": **אפס** ממצאי ניגודיות ו**אפס** ממצאי לשון.
  הממצא היחיד הוא "+ הוסיפי ספר" — "ספר" כשם עצם, תעתוע ידוע ומתועד.

**מה שנשאר פתוח:**

- **שמירה שנכשלת אינה אומרת דבר.** `saveSummary` הוא
  `try{localStorage.setItem(...)}catch(e){}` — אם האחסון מתמלא, הרפלקציה שלה
  לא נשמרת והמסך מתקדם כאילו כן. נמדד לפני שנבהלתי: יום מלא הוא בסדר גודל
  של קילובייטים בודדים ותקרת localStorage היא כ-5MB, כלומר שנים. זה גג אמיתי
  אך רחוק, והתיקון הנכון אינו שורה אלא מצב — מה האפליקציה אומרת כשהיא לא
  יכולה לשמור. עיצוב, לא תיקון תקלה.

- **טיוטה לתובנה, אם נרצה.** ביקורת על כל מה שאפשר לאבד מצאה 19 תיבות טקסט,
  שרק שלוש מהן נושאות `oninput` — ומהן רק היומן ו-fdb שומרים באמת. הרפלקציה
  והיומן נסגרו היום. `ins-text` — "התובנה שלך" — הוא השדה החופשי הארוך
  הנוסף, והוא נשמר רק בלחיצה על "שמור". אבל שם התיקון אינו אותו תיקון:
  ברפלקציה לתשובה יש שאלה ידועה ו"המשיכי מאיפה שעצרת" הוא פיצ'ר מתוכנן,
  ואילו תובנות הן רשימה — שמירה אוטומטית של חצי מחשבה תופיע אצלה כרשומה.
  מה שצריך שם הוא טיוטה, לא רשומה, וזה עיצוב ולא תיקון תקלה.
  שאר 15 התיבות הן שדות קצרים בתוך מגירות שכפתור השמירה נמצא בהן.


- **כרטיס הגיבוי במסך הבית, וכפתור "ייצוא" שבתוכו.** סריקת הצבע מצאה אותו
  ב-`#home-backup`, גרדיאנט סגול, כ-900 פיקסלים מתחת לקו הקיפול. הוא לא
  הטבעת, אבל הוא כן מסך הבית, ואסף אמר במפורש לא לגעת בו — אז הוא רשום
  ולא הוסב. שאלה אליו, לא החלטה שלי.

**ומה שההליכה בדלת מצאה, מדלת ריקה ועד סיום יום.** שני דברים, ושניהם רק
בדקה הראשונה שלה — כלומר בדיוק במקום שאף בדיקה חוזרת לא מגיעה אליו.

הדלת שאלה אילו אזורים היא רוצה, הבטיחה "אלה יהיו מה שמופיע מסביב לסיום
היום", כתבה את התשובה ונסגרה — ומסך הבית שמאחוריה נבנה באתחול, לפני
שהשאלה בכלל נשאלה. היא נחתה על טבעת שנושאת את כל שבעת האזורים כולל אלה
שכיבתה, ועל ברכה ששואלת "מי את?" אחרי שהקלידה את שמה. זה תיקן את עצמו
בציור הבא, כלומר בדקה השנייה שלה.

ואז המסך הראשון של הרפלקציה נפתח ב**"בוא** ניקח רגע" — לשון זכר, אחרי
שכל מסכי הדלת דיברו אליה. find-masculine-left דיווח אפס כי רשימת המילים
שלו לא הכירה את "בוא". שאילת מאגר המפתחות במקום הרשימה החזירה שלוש מילים
חסרות — בוא, תן, שים — וארבעה מפתחות; ואחת שפסלתי בעצמי עשר דקות קודם,
כי ראיתי ש"ספר" הוא שם עצם בארבעה מפתחות ולא שמתי לב שבחמישי הוא פועל:
"ספר לי מה יש לך בשבוע". השכבה עלתה מ-206 ל-211.

**ואז ההליכה נכנסה לרפלקציה עצמה.** השאלה הראשונה של הערב מציירת דילוג
שאומר "דלגי" והמשך שאומר "המשך" — אחד בנקבה ואחד לא, סנטימטר זה מזה.
וזו המילה השלישית באותו יום שהיא גם פועל וגם שם עצם, והפעם שתי המשמעויות
חיות באותה אפליקציה: המשך הוא הכפתור, והמשך הוא גם "המשך יום טוב". הכלל
בגנרטור יורה רק בפתיחת פסוקית, מה שמרחיק ממנו את "שיהיה המשך" ו"שבהם
המשך"; הברכה שפותחת מחרוזת משלה נקובה ב-KEEP, כי בדיוק הטעות ההיא כבר
רשומה כאן — מעבר קודם ייצר "המשיכי פוגע".

ההליכה המשיכה ומצאה עוד שלוש: הראה (המסך האחרון של הרפלקציה), שאל
(הכפתור שמפעיל את העוזר) ותאר (שני טקסטים מנחים). ושלוש שלא נכנסו,
ובכוונה לא דרך רשימת היתר: "קרא" מנקב את הפעולה של העוזר עצמו בכל
המפתחות שמחזיקים אותה, והקבוצה גדלה עם כל כלי חדש. רשימת היתר שחייבת
לגדול עם פיצ'ר היא רשימה שנרקבת.

**ואז המחלקה נסגרה בבת אחת, במקום עוד מילה בכל הליכה.** ארבע ההליכות
מצאו כל אחת מילה אחת, ורק במקום שבמקרה הסתכלתי בו. אז במקום להמשיך
ללכת שאלתי את מאגר המפתחות שאלה חסומה: לכל מפתח שהשכבה לא עונה עליו,
מהי המילה העברית הראשונה. 821 מילים שונות — רשימה שאדם קורא פעם אחת.
עשרים ואחת מהן ציווי אליה, ב-31 מפתחות, וכל אחת נבדקה מול *כל* מפתח
שמחזיק אותה ולא רק מול אלה שפותחים בה. השכבה: 221 ← 257.

והבדיקה לא הייתה שבורה, אף שנראתה כך: "הסר" עומד בראש חמישה מפתחות שאינם
בשכבה והיא הייתה ירוקה. הלוגיקה נכונה — המילה פשוט לא הייתה ברשימה שלה,
ליד "הוסף" שכן.

**עיוורון שראיתי בסורק הצבע, ונמדד עד הסוף:** הסורק קורא רק את מה שמצויר
ברגע הזה, אז אלמנט שמאחורי תנאי נעלם ממנו. שאלתי את המקור במקום, לפי
פונקציה, וזה מה שיש: **128 אזכורים של הפלטה הישנה בתוך מארקאפ שנבנה
ב-JavaScript, ב-48 פונקציות.** הסריקה הכריזה על שבעה אזורים נקיים והם
היו שם כל הזמן, מאחורי תנאים.

הכבדים, לפי סדר:

    18  renderEditProgram      עריכת תוכנית אימון
     9  renderFitWorkout       מסך האימון החי
     6  planDayHTML            התכנון היומי
     5  renderToday            תזונה — המסך הראשי
     5  renderInterview        · 5 openCalDay · 5 renderRecipeList
     5  renderViewRecipe       · 4 renderFitStepsSetup · 4 renderSumRem

מה שנסגר עד כה מתוכם: הרפלקציה (4). מה שנשאר הוא בעיקר כושר ומתכונים,
ואחריהם תכנון זמן ותזונה.

**ואיך לא להמיר אותם:** לא במיפוי טוקן. זה מה שעשיתי ב"אני" והוא הוליד
תאריך בלתי קריא — `--violet-500` מופה לענבר, ו-`cal-d.today` שמצייר
טקסט בטוקן הזה יצא 1.63:1 מול הרקע. טוקן שמשמעותו "הדגש" לא יכול להיות
גם "טקסט". מחלקות, אחת-אחת, ומדידת ניגודיות אחרי.

**והמספר הזה מוגזם, כי שני המכשירים טועים לכיוונים הפוכים.** סריקת הצבע
קוראת רק את מה שמצויר, אז היא מפספסת כל ענף שמאחורי תנאי. מפת המקור
קוראת כל אזכור, אז היא סופרת גם מארקאפ שכבר אינו מצויר לעולם. חמישה
מתוך ה-126 הם ב-`renderToday`, וארבעה מהם בבלוק `.pills` — פריסה ישנה
של ארבעת המאקרו שהעיצוב החדש של תזונה החליף, ושיושבת מאחורי
`body.mod-nutrition .pills{display:none}`. מדדתי שם ניגודיות של 1.72:1
ועמדתי לדווח עליה, עד שבדקתי אם היא בכלל מצוירת. היא לא.

**וסריקת ניגודיות על כל מילה מצוירת בכל אזור** — שמרכיבה את הרקע האמיתי
שכבה אחר שכבה — סגרה שש כתוביות שנשארו מצביעות על הטוקנים שלפני החומר,
והגרועה בהן הייתה הוראה: "החלק להחלפת שבוע", 1.59:1 בתשעה פיקסלים, המקום
היחיד שהלוח מסביר בו איך מפעילים אותו. שמונת האזורים נקיים עכשיו.

**נסגר ב-25 בספטמבר, אחרי שאסף אמר "להרים".** `--muted` עבר ל-#6d675e
(שהוא בדיוק `--nu-muted`) ו-`--muted-soft` ל-#7a6f68. נמדד אחרי: שמונת
האזורים והלשוניות הפנימיות של תזונה נקיים לגמרי, ומסך האימון החי ירד
מארבעים ממצאים לשמונה.

**ומה שנשאר הוא מחלקה אחרת, לא הטוקן הזה:** טקסט לבן על מילוי צבעוני —
"Bench Press" על גרדיאנט הטרקוטה ב-1.71:1, "סיימי אימון ✓" ו"התחילי
מנוחה" ב-1.43:1, ומספרים גדולים בלשונית ההיסטוריה שצבועים בצבע הסדרה
שלהם. זו החלטה על צבעי המילוי עצמם, לא על צבע הכתוביות.

---

*מה שהיה כתוב כאן קודם, ששימש לקבלת ההחלטה:*

**והמדידה הושלמה, וזו החלטה אחת שלך ולא מאה תיקונים שלי.**

`--muted` משמש כצבע טקסט ב-**392** מקומות ו-`--muted-soft` ב-**111**.
נמדד: 2.12:1 על הרקע הרך ו-2.44 על הקרם; muted-soft 1.59 ו-1.83. זה לא
משתנה לפי מסך — זו ההיררכיה של האפליקציה, והיא הייתה מתחת לקו גם לפני
החומר הרך.

המועמד: `--muted:#6d675e` (זה בדיוק `--nu-muted`, 4.53 / 5.20) ו-
`--muted-soft:#7a6f68` (3.95 / 4.54, ונשאר צעד מתחתיו).

נוסה בדפדפן בלבד, בלי לגעת בקובץ, על לשונית "אישי" בתזונה: **24 ממצאים
ירדו ל-0** משינוי שני ערכים, וההבדל הוויזואלי קטן — אותה פריסה, אותו
משקל, רק קריא. שתי צילומי מסך נלקחו לפני ואחרי.

לא שיניתי. זה משנה את המראה של כחמש מאות אלמנטים בכל האפליקציה, וזו
החלטה של אסף.

**ומה שנשאר, ובכוונה לא נגעתי בו:** מסך האימון החי מחזיר ארבעים ממצאים,
כמעט כולם בדיוק 2.12:1, כי הוא צובע כמעט כל תווית קטנה ב-`--muted`. זו לא
שארית המרה — זה צבע הכתוביות של האפליקציה כפי שנבחר, והוא היה 2.43:1 גם
על הקרם שלפני החומר. להרים טוקן שכל האפליקציה נשענת עליו זו החלטה של אסף,
לא תיקון תקלה. אם כן נרצה: `--nu-muted` הוא 4.53:1 והוא כבר הוכח על שמונה
מסכים.

**ושתי מגבלות של סורק הניגודיות, כדי שלא ייקרא כאמת:** גרדיאנט הוא רקע
(הוא קרא רק backgroundColor והחזיר 1.23:1 על טקסט לבן שיושב על טרקוטה),
ושכבה `position:fixed` מאחורי התוכן אינה אב — במסך הבית הוא מרכיב לבן
ומחזיר "לילה טוב" ב-1.14:1, כשבפועל זו לבנדר בהירה על שמי לילה. שתיהן
תוקנו או נרשמו לפני שדיווחתי משהו.

**ומה שנמצא כשסרקתי את כושר באותה שאלה:** ארבעה טקסטים על מסך האימון החי
מתחת ל-2.5:1 — הדרך חזרה, מונה התרגילים, ושתי תוויות ב-8.5 פיקסלים. הצבע
נכון: טרקוטה הוא הצבע של כושר, ומידע. הערך לא. ומעל לזה, האפליקציה כבר
החזיקה טוקן לזה — `--terra-700` עם הערה שאומרת "readable as text" —
שנמדד ב-3.21:1 בשלושה-עשר מקומות שסמכו עליה. ערך אחד תוקן ל-#98492c,
והכול עבר 5:1. זה ההפך ממה שעשיתי ב"אני": שם מיפיתי טוקן קיים למשמעות
אחרת, וכאן הטוקן כבר התכוון להיות טקסט ורק ערכו לא עמד בזה.

**ושניים שנמדדו ונמצאו מכוונים, ולכן לא הומרו:**

- **הסיכום היומי הוא "פריסת הנייר".** כרטיסים לבנים, Georgia, קווים
  מפרידים — עיצוב שנבחר, ו-CLAUDE.md מזכיר אותו במפורש ברשימת ההיתר של
  בדיקת האנגלית. אין להמיר אותו לחומר הרך בלי החלטה של אסף.
- **ארבעת צבעי המאקרו הם מידע.** בסיכום ובתזונה כאחד הם הדבר היחיד
  שמבדיל בין קלוריות, חלבון, פחמימה ושומן — אותו מקרה כמו שבעת האייקונים
  בתובנות.

**מה שעוד לא נסגר בשכבה, ורשום כאן כדי שלא יישכח:** היתר ב-find-masculine-left
פוטר מפתח שלם ולא את המילה שבשבילה נכתב. מפתח שהותר מסיבה אחת בלתי נראה
אם יתברר שהוא נושא מילה אחרת. אחד כבר עשה את זה — "שאל על אוכל" נמצא שם
בגלל "אוכל" ואיש לא שם לב ש"שאל" הוא ציווי. הגנרטור סובב אותו בכל זאת,
אז זה לא שוחרר, אבל שום דבר לא היה אומר את זה. היתר צריך לנקוב במילה.

**ומה שהספר הסתיר.** הצביעה שלו הייתה הסיבה להיכנס אליו, ומה שנמצא בפנים
היה גדול ממנה. סימן האחוז היה מולחם ביד בארבעה מקומות — לא בקוד אלא בתוך
המפתח, `_t('% הושלם')`, שם הבדיקה לא יכלה לראות אותו; היא הורחבה, מוצאת 4
מול הגרסה הפגומה ו-0 מול התיקון. מספר העמוד נכתב גולמי לצד סך שכן עבר
מעצב, והספרייה הדפיסה תאריך ISO כפי שאוחסן. והחמור מכולם, נמדד על היציב
החי: אי אפשר היה ללחוץ "שמור" באף אחת משתי המגירות — `#me-screen` נושא
`z-index:1`, שלוכד בתוכו כל z-index שנכתב בפנים, והסרגל נצבע מעליהן בכל
גודל מסך.

**ואיך נמצאו השלושה שכן הוסבו היום:** לא ב-grep אלא בסריקה של מה שנצבע.
לכל אלמנט בכל אזור נשאל אם הצבע שלו מתיישב על הפלטה של העולם הקודם — סגול
וטרקוטה — ולא אם הטוקן כתוב במקור. זה תופס צבע שרותך לתוך `style="…"`,
שאין כלל CSS שמגיע אליו, וזה בדיוק מה שהיה: כפתור "בית" הוא אתר אחד
במארקאפ ויושב בכותרת של כל תשעת האזורים. ההרצה הראשונה דיווחה שמונה
ממצאים כמעט בכל אזור — כולם ספלאש האזור, שעדיין על המסך 1.5 שניות אחרי
`enterModule`. אותה טעות מדידה שכבר עלתה כאן פעמיים. אחרי ההמתנה: שבעה
אזורים נקיים לגמרי, ומה שנשאר הוא צבע קטגוריה בתוך אייקון קו — מידע, ולכן
נכון שהוא צבעוני.
- **ההמתנה שנשארה במתכנן.** שלב 1 נעשה קלף מקומי וזוג העוגנים התמזג, אבל
  כל תשובה משלב 2 והלאה עדיין עולה 2 עד 4 שניות, כי היא שואלת מודל. אין
  לזה פתרון מקומי — רק פחות שאלות.
- **מחרוזת חדשה שפונה אליה חייבת לעבור את find-masculine-left.** הבדיקה
  ירוקה היום — 206 מפתחות בשכבה, אפס פתוחים — והיא תישאר ירוקה רק אם כל
  מחרוזת חדשה נבדקת מולה. היא לא רצה לבד.
- **מסך ה״ניתוח״ מהמפרט של אסף נדחה בכוונה.** ה-Labor Illusion עובד, אבל
  בר שרץ מאפס למאה על עבודה שלא קורית הוא הדבר הראשון שהאפליקציה אומרת
  ואינו נכון, והוא נאמר למישהי שרק נכנסה. אם נרצה את הרגע הזה — שיאמר מה
  באמת נעשה: מה נשמר, מה יופיע, ושהיעדים התחלתיים.
...
> **25 בספטמבר — הסיבה הזאת פגה.** "אין דפוס אישור פנימי" היה נכון כשזה
> נכתב ואינו נכון מאז שהחומר הרך הוכרע: קלף מורם, אור מתחת לפעולה, והנגדית
> לחוצה. `askConfirm` נבנה מהדפוס הזה ואינו החלטת עיצוב חדשה. ארבעה
> `confirm()` עברו אליו — מחיקת אימון, החלפת אימון פעיל, הסרת תרגיל וסיום
> ספר; המטרות שברשימה כאן כבר אינן במוצר. אפס מפתחות חדשים: מחק, הסר,
> להתחיל, סיימתי וביטול כבר נענו באחת-עשרה שפות, וכל אחד מהם מנקב את
> הפעולה במקום לומר "אישור". ולמה זה לא היה רק סגנון: דיאלוג יליד כותב את
> שני הכפתורים שלו בשפת **הדפדפן**, לא בשפת האפליקציה — קוראת יפנית שמכשירה
> באנגלית נשאלה בעברית ונענתה ב-OK/Cancel.
