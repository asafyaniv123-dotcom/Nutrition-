...
- **הווידג'ט של הספר הוא עולם משלו בתוך style="".** כריכה, סליידר, ציטוטים,
  ספרייה ושני מגירות — כמעט כולו סגול שכתוב בתוך תכונות, שאין כלל CSS
  שמגיע אליו. בתוך `mod-me` הטוקן `--violet-500` הוסב לענבר, אז מה שקורא
  אותו כבר בחומר; `--violet-300` וכרטיסי הציטוט שנכתבו בצבע מפורש לא.
  המצב הריק — היחיד שהמשתמשת הראשונה תראה מחר — הומר במלואו. השאר הוא מעבר
  בפני עצמו, וצריך להתחיל בהחלפת המחרוזות המוטבעות במחלקות, בזהירות: שלוש
  מהן הן מה ש-bookProgress מוצא לפיו את הבר ואת האחוז.
- **ההגדרות שמאחורי גלגל השיניים עדיין בעולם הישן.** `.set-lang.on` נצבע
  ב-`--violet-500` ועל `--violet-50`. זה המסך היחיד שנשאר שאפשר להגיע
  אליו ולא נגענו בו.
