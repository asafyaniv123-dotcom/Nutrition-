...
- **ארבעה `confirm()` ילידיים נשארו, ו-`prompt(` אחד.** שלושה בכושר —
  מחיקת אימון, התחלת אימון חדש, הסרת תרגיל — ואחד בספר. TODO רשם שהדיאלוגים
  הילידיים הוסרו; מה שהוסר היה ה-`alert()`-ים. דיאלוג יליד עוצר את הדף,
  אינו מדבר בשפת הקורא ואינו נראה כמו האפליקציה, ובדפדפן אוטומטי הוא מקפיא
  את הכרטיסייה. זה מעבר אחד משותף לארבעתם, לא תיקון לכל אתר.
