...
- **find-duplicate-options לא רואה רשימות שנבנות בזמן ריצה.** הוא קורא
  רשימות שמוכרזות יחד בקוד, אז המילה tomorrow באות קטנה בין This week
  ל-Three months עברה דרכו. מצאתי את זה בעין, ואותה עיוורון קיימת לכל
...
- **ההוראה ב-CLAUDE.md ששומרת על המשחק שומרת על משהו שאיננו.** game-src
  מופיע אפס פעמים בשני העותקים. הבדיקה שכתבתי לשער השחרור החזירה "זהה"
  על אורך אפס, וזה בדיוק סוג הפאס הריק שהקובץ הזה מזהיר מפניו.
